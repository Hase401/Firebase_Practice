...
final class FileViewController: UIViewController {
    ...
    private let fileDateRepository =  FileDateRepository()
...
private extension FileViewController {
    ...
    func showCurrentFileDates() {
        fileDateRepository.fetchFileDate(folderDate: self.folderDate) { [weak self] response in
            ...
            switch response {
            case .success(let fileDates):
                self.fileDates = fileDates
                self.addFileHandler(self.fileDates.count)
    ...
    @objc func addFileButtonTapped() {
        ...
        let newFileByTemplate = UIAlertAction(title: "テンプレートから新規作成",
                               ...
                               handler: { (action: UIAlertAction!) in
                                ...
                                self.fileDateRepository.addFileDate(folderDate: self.folderDate, fileDate: newFileDate) { [weak self] response in
                                    guard let self = self else { return }
                                    switch response {
